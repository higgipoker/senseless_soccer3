...
void Camera::update() {
  if (following) {
    float distance =
        (movable.position - following->movable.position).magnitude2d();
      if (Math::greater_than(distance, 100)) {
        movable.velocity = following->movable.velocity;
      }
  ...
  if (movable.position.x + view.getSize().x / 2 > world.left + world.width) {
    movable.setPosition(world.left + world.width - view.getSize().x / 2,
                        movable.position.y);
  }
  ...
  if (movable.position.y + view.getSize().y / 2 > world.top + world.height) {
    movable.setPosition(movable.position.x,
                        world.top + world.height - view.getSize().y / 2);
  }
...
//
//
void Camera::setWorldRect(sf::FloatRect in_rect) { world = in_rect; }
}  // namespace Engine
