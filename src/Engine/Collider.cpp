...
//
//
bool Collider::contains(const sf::CircleShape &big, const sf::CircleShape &small) {
    // distance
    int dist_sq = sqrt(((small.getCenter().x - big.getCenter().x) * (small.getCenter().x - big.getCenter().x)) +
                       ((small.getCenter().y - big.getCenter().y) * (small.getCenter().y - big.getCenter().y)));
...
// -----------------------------------------------------------------------------
bool Collider::collides(const Engine::Vector3 &point, const sf::TriangleShape &triangle) {
    ...
    Vector3 v1 = Vector3(triangle.getPoint(1).x, triangle.getPoint(1).y) -
                 Vector3(triangle.getPoint(0).x, triangle.getPoint(0).y);
    Vector3 v2 = Vector3(point.x, point.y) - Vector3(triangle.getPoint(0).x, triangle.getPoint(0).y);
