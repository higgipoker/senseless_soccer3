...
//
//
GameEngine::GameEngine(Camera &in_camera, const std::string &in_window_title,
                       float in_window_width, float in_window_height,
                       int in_flags, bool in_fullscreen)
...
void GameEngine::step() noexcept {
  ...
  for (auto &layer : render_layers) {
    // sort the layer
    if (layer.sortable) {
      std::sort(std::begin(layer.sprite_list), std::end(layer.sprite_list),
...
//
//
int GameEngine::addLayer(const bool in_sortable,
                         const bool in_overlay) noexcept {
...
void GameEngine::add_sprite(Sprite &in_sprite, layer_id in_layer_id) noexcept {
  ...
  if (in_layer_id == RenderLayer::INVALID_LAYER) {
    ...
    if (in_layer_id < render_layers.size()) {
      ...
      std::cout << "addRenderable> Tried to add to non existent layer: "
...
//
//
layer_id GameEngine::getDefaultLayer() const noexcept { return sprite_layer; }
//
//
//
