...
void GameEngine::step() {
  ...
  if (Debug::showHud()) {
    window.setView(hud_view);
    debug_gui.update(static_cast<int>(frame_counter),
                     static_cast<int>(frametime));
    window.setView(camera.getview());
    picker.update();
  }
...
int GameEngine::addLayer(bool in_sortable) {
  const int new_id = static_cast<unsigned int>(render_layers.size());
...
void GameEngine::addSprite(Sprite &in_sprite, layer_id in_layer_id) {
  ...
  if (in_layer_id == RenderLayer::INVALID_LAYER) {
    ...
    render_layers[default_layer].sprite_list.push_back(&in_sprite);
  } else {
    // add to specified layer
    if (in_layer_id < render_layers.size()) {
      render_layers[in_layer_id].sprite_list.push_back(&in_sprite);
    } else {
...
//
//
layer_id GameEngine::getHudLayer() const { return hud_layer; }
//
//
//
