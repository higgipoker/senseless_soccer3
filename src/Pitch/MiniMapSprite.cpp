...
void MiniMapSprite::init(const PitchDrawDimensions& in_dimensions) {
...

  {
    // south 6 box
    // |
    south_6_box[0] = {{in_dimensions.box_6.getPosition().x,
                       in_dimensions.box_6.getPosition().y + in_dimensions.box_6.getSize().y}};
    south_6_box[1] = {{in_dimensions.box_6.getPosition().x, in_dimensions.box_6.getPosition().y}};

    // _
    south_6_box[2] = south_6_box[1];
    south_6_box[3] = {
        {south_6_box[1].position.x + in_dimensions.box_6.getSize().x, south_6_box[1].position.y}};
    ...
    const float dis_from_box_to_box =
        in_dimensions.bounds.getSize().y - (in_dimensions.box_6.getSize().y * 2);
...

  {
    // south 18 box
    // |
    south_18_box[0] = {{in_dimensions.box_18.getPosition().x,
                        in_dimensions.box_18.getPosition().y + in_dimensions.box_18.getSize().y}};
    south_18_box[1] = {
        {in_dimensions.box_18.getPosition().x, in_dimensions.box_18.getPosition().y}};

    //    // _
    south_18_box[2] = south_18_box[1];
    south_18_box[3] = {{south_18_box[1].position.x + in_dimensions.box_18.getSize().x,
    ...
    const float dis_from_box_to_box =
        in_dimensions.bounds.getSize().y - (in_dimensions.box_18.getSize().y * 2);
  ...
  vlines = {// bounds
            ...
            // south 6 yard box
            south_6_box[0], south_6_box[1], south_6_box[2], south_6_box[3], south_6_box[4],
            south_6_box[5],

            // north 6 yard box
            north_6_box[0], north_6_box[1], north_6_box[2], north_6_box[3], north_6_box[4],
            north_6_box[5],

            // south 18 yard box
            south_18_box[0], south_18_box[1], south_18_box[2], south_18_box[3], south_18_box[4],
            south_18_box[5],

            // north 18 yard box
            north_18_box[0], north_18_box[1], north_18_box[2], north_18_box[3], north_18_box[4],
            north_18_box[5],
...
//
//
void MiniMapSprite::updatePlayerPositions(const std::vector<Vector3>& in_positions,
...
sf::FloatRect MiniMapSprite::getGlobalBounds() const {
  return {getPosition().x, getPosition().y, bounds[1].position.x * getScale().x,
          bounds[3].position.y * getScale().y};
}
//
//
//
