...
Pitch::Pitch(const std::string& in_grass_texture, const sf::FloatRect in_bounds) : minimap_entity(std::move(minimap)) {
    ...
    static_cast<MiniMap*>(&minimap_entity.getSprite())->init(in_bounds, draw_dimensions);
...
void Pitch::init_bounds() {
    ...
    draw_dimensions.bounds.setSize(sf::Vector2f(static_cast<float>(Metrics::MetersToPixels(69)),
    ...
    draw_dimensions.draw_bounds_north.setSize(sf::Vector2f(static_cast<float>(Metrics::MetersToPixels(69)),
                                                           static_cast<float>(Metrics::MetersToPixels(105))));
    ...
    draw_dimensions.draw_bounds_north.setSize(
        {draw_dimensions.draw_bounds_north.getSize().x, draw_dimensions.draw_bounds_north.getSize().y / 2});
    ...
    draw_dimensions.draw_bounds_south.setSize(sf::Vector2f(static_cast<float>(Metrics::MetersToPixels(69)),
                                                           static_cast<float>(Metrics::MetersToPixels(105))));
    ...
    draw_dimensions.draw_bounds_south.setSize(
        {draw_dimensions.draw_bounds_south.getSize().x, draw_dimensions.draw_bounds_south.getSize().y / 2});

    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::Bye)]
                     [static_cast<size_t>(PitchPointOfInterestSide::South)] = {0, 0};
    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::Bye)][static_cast<size_t>(
        PitchPointOfInterestSide::North)] = {0, static_cast<float>(Metrics::MetersToPixels(105))};

    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::SideWest)]
                     [static_cast<size_t>(PitchPointOfInterestSide::South)] = {0, 0};
    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::SideWest)]
                     [static_cast<size_t>(PitchPointOfInterestSide::North)] = {0, 0};

    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::SideEast)][static_cast<size_t>(
        PitchPointOfInterestSide::South)] = {static_cast<float>(Metrics::MetersToPixels(69)), 0};
    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::SideEast)][static_cast<size_t>(
        PitchPointOfInterestSide::North)] = {static_cast<float>(Metrics::MetersToPixels(69)), 0};
}
//
//
//
void Pitch::init_6_yard_boxes() {
    ...
    float x = (draw_dimensions.bounds.getSize().x / 2) - (draw_dimensions.south_6.getSize().x / 2);
    float y = draw_dimensions.bounds.getSize().y - draw_dimensions.south_6.getSize().y;
    draw_dimensions.south_6.setPosition(sf::Vector2f(x, y));

    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::Six)]
                     [static_cast<size_t>(PitchPointOfInterestSide::South)] = {x, h};

    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::Six)][static_cast<size_t>(
        PitchPointOfInterestSide::North)] = {x, dimensions
...
void Pitch::init_18_yard_boxes() {
    ...
    float x = (draw_dimensions.bounds.getSize().x / 2) - (draw_dimensions.south_18.getSize().x / 2);
    float y = draw_dimensions.bounds.getSize().y - draw_dimensions.south_18.getSize().y;
    draw_dimensions.south_18.setPosition(sf::Vector2f(x, y));

    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::Eighteen)]
                     [static_cast<size_t>(PitchPointOfInterestSide::South)] = {x, h};

    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::Eighteen)][static_cast<size_t>(
        PitchPointOfInterestSide::North)] = {x, dimensions
                                                        .points[static_cast<size_t>(PitchPointsOfInterest::Bye)]
                                                               [static_cast<size_t>(PitchPointOfInterestSide::North)]
                                                        .y -
...
void Pitch::init_halfway_line() {
    ...
    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::Halfway)]
                     [static_cast<size_t>(PitchPointOfInterestSide::South)] = {x, y};
    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::Halfway)]
                     [static_cast<size_t>(PitchPointOfInterestSide::North)] = {x, y};
}
//
//
//
void Pitch::init_center_circle() {
    ...
    draw_dimensions.center_circle.setRadius(static_cast<float>(Metrics::MetersToPixels(9.15f)));
...
void Pitch::init_center_spot() {
    draw_dimensions.center_spot.setRadius(static_cast<float>(Metrics::MetersToPixels(0.2f)));
    ...
    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::CenterSpot)]
                     [static_cast<size_t>(PitchPointOfInterestSide::South)] = {draw_dimensions.bounds.getSize().x / 2,
                                                                               draw_dimensions.bounds.getSize().y / 2};

    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::CenterSpot)]
                     [static_cast<size_t>(PitchPointOfInterestSide::North)] = {draw_dimensions.bounds.getSize().x / 2,
                                                                               draw_dimensions.bounds.getSize().y / 2};
}
//
//
...
void Pitch::init_penalty_spots() {
    draw_dimensions.south_penalty_spot.setRadius(static_cast<float>(Metrics::MetersToPixels(0.2f)));
    ...
    dimensions.points[static_cast<size_t>(PitchPointsOfInterest::PenaltySpot)][static_cast<size_t>(
        PitchPointOfInterestSide::South)] = {x, draw_dimensions.bounds.getSize().y - y};
}
//
//
//
void Pitch::init_south_arc() {
    ...
    draw_dimensions.south_arc.setRadius(static_cast<float>(Metrics::MetersToPixels(7.f)));
    ...
    draw_dimensions.south_arc.setOrigin(
        sf::Vector2f(draw_dimensions.south_arc.getRadius(), draw_dimensions.south_arc.getRadius()));
...
//
//
Engine::Vector3 Pitch::getPointOfInterest(const PitchPointsOfInterest in_which,
