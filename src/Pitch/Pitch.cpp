...
Pitch::Pitch (const std::string &in_grass_texture) : Entity (std::make_unique<Sprite>(), std::make_unique<Sprite>()) {
    ...
    name = "pitch";
    draw_shapes.origin = {250, 250, 0.F};
    init_bounds();
    init_halfway_line();
    init_center_circle();
    init_center_spot();
...
void Pitch::init_bounds() {
    ...
    //
    dimensions.points[BYE][NORTH] = {0, 0, 0};
    dimensions.points[BYE][SOUTH] = {0, PITCH_LENGTH, 0};

    dimensions.points[WEST][NORTH] = {0, 0, 0};
    dimensions.points[WEST][SOUTH] = {0, 0, 0};

    dimensions.points[EAST][NORTH] = {PITCH_WIDTH, 0, 0};
    dimensions.points[EAST][SOUTH] = {PITCH_WIDTH, 0, 0};

    north.p1 = {0.F, 0.F, 0.F};
    north.p2 = {PITCH_WIDTH, 0, 0.F};

    south.p1 = {0, PITCH_LENGTH, 0.F};
    south.p2 = {PITCH_WIDTH, PITCH_LENGTH, 0.F};

    east.p1 = {PITCH_WIDTH, 0, 0.F};
    east.p2 = {PITCH_WIDTH, PITCH_LENGTH, 0.F};

    west.p1 = {0, 0, 0.F};
    west.p2 = {0, PITCH_LENGTH, 0.F};
}
//
//
//
void Pitch::init_6_yard_boxes() {
    ...
    //
    dimensions.points[SIX][NORTH] = {x, y + SIX_YARD_BOX_LENGTH, 0};
    dimensions.points[SIX][SOUTH] = {x, dimensions.points[SIX][NORTH].y + (draw_shapes.bounds.getSize().y - SIX_YARD_BOX_LENGTH * 2), 0};
}
//
//
//
void Pitch::init_18_yard_boxes() {
    ...
    dimensions.points[EIGHTEEN][NORTH] = {x, y + EIGHTEEN_YARD_BOX_LENGTH, 0};
    dimensions.points[EIGHTEEN][SOUTH] = {x, dimensions.points[EIGHTEEN][NORTH].y + (draw_shapes.bounds.getSize().y - EIGHTEEN_YARD_BOX_LENGTH * 2), 0};
}
//
//
//
void Pitch::init_halfway_line() {
    ...
    dimensions.points[HALFWAY][NORTH] = {x, y, 0};
    dimensions.points[HALFWAY][SOUTH] = {x, y, 0};
}
//
//
//
void Pitch::init_center_circle() {
    ...
    dimensions.points[CENTER_CIRCLE_WEST][NORTH] = {
        ...
        draw_shapes.bounds.getSize().x / 2 - draw_shapes.center_circle.getRadius(), 0
    };
...
void Pitch::init_center_spot() {
    ...
    dimensions.points[CENTER_SPOT][NORTH] = dimensions.points[CENTER_SPOT][SOUTH] = {draw_shapes.bounds.getSize().x / 2,draw_shapes.bounds.getSize().y / 2, 0};
}
//
//
//
void Pitch::init_penalty_spots() {
    ...
    dimensions.points[PENALTY_SPOT][NORTH] = {x, y, 0};
    dimensions.points[PENALTY_SPOT][SOUTH] = {x, draw_shapes.bounds.getSize().y - y, 0};
}
//
//
//
...
//
//
sf::Vector3f Pitch::getPointOfInterest (const PitchPointsOfInterest in_which,
