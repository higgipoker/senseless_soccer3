...
//
//
bool MatchFactory::createMatchTexture(UniquePtr<Engine::Texture> team1_texture,
    ...
    auto width = std::max(
        ...
        std::max(team2_texture->getSize().x, std::max(shadow_texture1->getSize().x, ball_texture->getSize().x)));

    auto height = team1_texture->getSize().y + team2_texture->getSize().y + shadow_texture1->getSize().y +
