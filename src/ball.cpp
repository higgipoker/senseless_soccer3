...
int init_ball(Ball &ball) {
  ...
  ball.entity = e;
  get_ball_entity(ball).co_friction = 0.5f;
...
void update_ball(Ball &ball) {
  ...
  ball.collidable.setPosition(
      get_ball_entity(ball).position.x - ball.collidable.getRadius(),
      get_ball_entity(ball).position.y - ball.collidable.getRadius());
