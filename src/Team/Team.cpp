...
//
Team::Team(const std::string &in_name, const TeamType in_home_or_away,
           const Kit &in_kit)
...
void Team::update() {
  ...
  auto dist = (last_ball_position.magnitude2d() -
               match->getBall().movable.position.magnitude2d());
  if (Math::greater_than(fabs(dist), 50)) {
    gameplan.updateDefensiveLine(match->getPitch(), match->getBall(),
                                 attacking_goal);
    last_ball_position = match->getBall().movable.position;
  }

  defensive_line.setPosition(match->getPitch()
                                 .toScreenSpace(gameplan.getDefensiveLine())
                                 .toSfVector());
...
void Team::addDefaultPlayers(const Team &in_other_team) {
  ...
  UniquePtr<PlayingPosition> left_center_back =
      std::make_unique<PositionCenterBack>(match->getPitch(), *this,
                                           in_other_team);
  ...
  UniquePtr<PlayingPosition> left_back = std::make_unique<PositionFullBack>(
      match->getPitch(), *this, in_other_team);
  ...
  UniquePtr<PlayingPosition> left_center_mid =
      std::make_unique<PositionCenterMidfielder>(match->getPitch(), *this,
                                                 in_other_team);
  ...
  UniquePtr<PlayingPosition> left_midfielder =
      std::make_unique<PositionWideMidfielder>(match->getPitch(), *this,
                                               in_other_team);
  ...
  UniquePtr<PlayingPosition> left_center_forward =
      std::make_unique<PositionCenterForward>(match->getPitch(), *this,
                                              in_other_team);
  ...
  for (size_t i = 0; i < positions.size(); ++i) {
    UniquePtr<Player> player = std::move(
...
void Team::setMatch(Match &in_match) {
  ...
  // debugs
  defensive_line.setSize(
      {match->getPitch().getDimensions().bounds.getSize().x, 6});
  defensive_line.setFillColor({255, 0, 0, 50});
}
//
//
//
Direction Team::getAttackingGoal() const { return attacking_goal; }
//
//
//
Direction Team::getDefendingGoal() const { return defending_goal; }
//
//
//
std::vector<Vector3> Team::getPlayerPositions() {
  ...
  for (auto &p : players) {
    positions.push_back(
        (p->movable.position - match->getPitch().getDimensions().origin));
  }
  return positions;
}
//
...
//
//
AttackingState Team::getAttackingState() const { return attacking_state; }
