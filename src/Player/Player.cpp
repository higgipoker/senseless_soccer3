...
//
//
Player::Player(Match &in_match, const Team &in_my_team,
               const Team &in_other_team, UniquePtr<PlayerSprite> in_sprite,
...
void Player::goToSetPiecePosition(const Situation in_situation,
  ...
  if (auto position = playing_position.get()) {
    ...
    brain.locomotion.seek(position->getTargetPosition(
        in_situation, match.getBall(), in_pitch_side));
  }
}
