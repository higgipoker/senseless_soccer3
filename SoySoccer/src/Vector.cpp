...
void Vector3::normalise() {
    if (Math::equal(this->magnitude(), 0))
        return;
...
bool Vector3::equals(const Vector3 &v, float tolerance) {
    return (Math::equal(x, v.x, tolerance) && Math::equal(y, v.y, tolerance) && Math::equal(z, v.z, tolerance));
}
//
//
//
...
float Vector3::perpProduct(const Vector3 &rhs, bool right_normal) {
    float y_component; float x_component;
